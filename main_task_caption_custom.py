...
from dataloaders.dataloader_custom_caption import Custom_Caption_DataLoader 
...
def dataloader_custom_train(args, tokenizer):
    custom_dataset = Custom_Caption_DataLoader(
        json_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
        feature_framerate=args.feature_framerate,
...
def dataloader_custom_test(args, tokenizer, split_type="test"):
    custom_dataset = Custom_Caption_DataLoader(
        json_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
    ...
    return dataloader_custom, len(custom_dataset)
...
def eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=None, test_set=None):
    ...
    if args.datatype == "msrvtt":
        ...
        all_caption_lists = []
        sentences_dict = test_dataloader.dataset.sentences_dict
        video_sentences_dict = test_dataloader.dataset.data
        for idx in range(len(sentences_dict)):
            video_id, _ = sentences_dict[idx]
            sentences = video_sentences_dict[video_id]
            all_caption_lists.append(sentences)
...
DATALOADER_DICT["custom"] = {"train":dataloader_custom_train, "val":dataloader_custom_test}
