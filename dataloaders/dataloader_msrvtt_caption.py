...
class MSRVTT_Caption_DataLoader_1k_test(Dataset):
    """MSRVTT train dataset loader."""
    def __init__(
            self,
            csv_path,
        ...
        print('using msrvtt 1k test set...')
        self.csv = pd.read_csv(csv_path)
        self.data = json.load(open(json_path, 'r'))
        self.feature_dict = pickle.load(open(features_path, 'rb'))
        self.feature_framerate = feature_framerate
        ...
        # Test: retrieval 1k
        video_ids = [itm for itm in self.csv['video_id'].values]
        print('number of videos:', len(video_ids))
        split_dict = {"test": video_ids}
        choiced_video_ids = split_dict[split_type]
